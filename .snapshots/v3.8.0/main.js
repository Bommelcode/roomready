...
function createWindow() {
  const win = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: true,
      preload: path.join(__dirname, 'preload.js'),
      // Geen throttling bij background — anders vertraagt device-enumeration / mic-meter
      // / camera-preview zodra de operator naar een ander venster switcht.
  ...
  // Helper: maak previewWin aan op `target` display. Sluit eerst bestaande als die er is.
  function _openPreviewOn(target) {
  ...
  // Auto-open previewWin op nieuw aangesloten extern scherm. Alleen als:
  //  - er nog geen preview open is (anders heeft operator handmatig gekozen, niet overrulen)
  //  - de nieuwe display niet de primaire is (intern laptopscherm — daar willen we niet auto-fullscreenen)
  screen.on('display-added', (_e, newDisplay) => {
    try {
      if (previewWin && !previewWin.isDestroyed()) return
      const primary = screen.getPrimaryDisplay()
      if (!newDisplay || newDisplay.id === primary.id) return
      _openPreviewOn(newDisplay)
    } catch (e) {
      console.warn('[main] display-added auto-open error:', e.message)
    }
  })
...
# LET OP: GEEN backslash in het WQL LIKE-patroon. Windows PowerShell 5.1's CIM-
# parser verwerpt 'USB\VID_046D%' als "Invalid query" (de backslash geldt als
# escape). Met $ErrorActionPreference='SilentlyContinue' wordt die fout
# ingeslikt en komt de scan LEEG terug. Gebruik '%' i.p.v. '\' als scheiding.
...
`
  ...
  // ── Firmware resolvers ──────────────────────────────────────
  function httpJson(url, { method = 'GET', headers = {}, timeout = 10000, body = null } = {}) {
    return new Promise((resolve) => {
      try {
        const u = new URL(url)
  ...
  // Sluit secundaire vensters (preview op extern scherm, inventaris) mee wanneer
  // het hoofdvenster sluit — anders blijft previewWin op de tweede monitor staan
  // en vuurt window-all-closed niet, dus app blijft draaien zonder UI.
  win.on('close', () => {
    try { if (previewWin && !previewWin.isDestroyed()) previewWin.close() } catch (_) {}
    try { if (inventoryWin && !inventoryWin.isDestroyed()) inventoryWin.close() } catch (_) {}
  })
