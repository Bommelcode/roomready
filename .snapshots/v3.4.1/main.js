  ...
  function runPowerShellJson(script, timeoutMs = 30000) {
    return new Promise((resolve) => {
      execFile('powershell.exe',
        ...
        (err, stdout) => {
          if (err) return resolve({ ok: false, error: err.message, data: [] })
          ...
          try {
            ...
            resolve({ ok: false, error: 'JSON parse: ' + e.message, data: [] })
          }
        })
    })
  }
...
$devs = Get-CimInstance Win32_PnPEntity | Where-Object { $_.PNPDeviceID -like 'USB\\VID_046D*' }
